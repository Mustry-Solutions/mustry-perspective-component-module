...
- **Edge labels** — optional `data.edgeLabels` (`[{from, to, label}]`) tags connectors (e.g. Yes/No on a decision branch); each entry targets an existing edge by its endpoint ids, and edges without an entry stay bare. Labels sit on the connector mid-point.
