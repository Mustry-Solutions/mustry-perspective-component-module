...
export function GridToolbar(p: GridToolbarProps): React.ReactElement {
    const { labels } = p;
    return (
        <div className="mustry-dg-toolbar">
            <input
                type="text"
                className="mustry-dg-search"
                value={p.filter}
                placeholder={labels.search}
                aria-label={labels.search}
                onChange={p.onFilter}
            />
            {p.filter && <span className="mustry-dg-count">{p.matchCount}</span>}
            <span className="mustry-dg-toolbar-spring" />
            {p.selCount > 0 && (
                <span className="mustry-dg-selected-badge">{labels.selected.replace('{n}', String(p.selCount))}</span>
            )}
            {p.batchMode && p.dirtyCount > 0 && (
                <>
                    <span className="mustry-dg-dirty-badge">{labels.unsaved.replace('{n}', String(p.dirtyCount))}</span>
                    <button type="button" className="mustry-dg-save-btn" onClick={p.onSaveBatch}>{labels.save}</button>
                    <button type="button" className="mustry-dg-export-btn mustry-dg-discard-btn" title={labels.discard}
            ...
            {p.allowAdd && (
                <button type="button" className="mustry-dg-export-btn" title={labels.addRow}
            ...
            {p.allowDelete && (
                <button type="button" className="mustry-dg-export-btn" disabled={p.selCount === 0}
            ...
            <button type="button" className="mustry-dg-export-btn mustry-dg-chooser-btn" title={labels.columns}
            ...
            {p.showExport && (
                <button type="button" className="mustry-dg-export-btn" title={labels.exportCsv}
