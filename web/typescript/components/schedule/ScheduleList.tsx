...
export function ScheduleList(props: ScheduleListProps): JSX.Element {
    const { items, selectedName, activeFlags, labels, onSelect } = props;
    return (
        <div className="mustry-sched-list" role="listbox" aria-label={labels.listHeader}>
            <div className="mustry-sched-list-header">{labels.listHeader}</div>
            {items.length === 0 && <div className="mustry-sched-empty">{labels.noSchedules}</div>}
            {items.map((s, i) => (
                <div
                    key={`${s.name}-${i}`}
                    className={'mustry-sched-item' + (s.name === selectedName ? ' mustry-sched-item--selected' : '')}
                    role="option"
