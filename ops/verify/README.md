...
| `/timeline-cycle` | TimelineCycle | Cycle-time fixture (issue #117): machine cycles with second-long (and sub-second) phases seeded around now; `config.zooms` offers the `second`/`minute` presets, opens at `second` on the current cycle. |
